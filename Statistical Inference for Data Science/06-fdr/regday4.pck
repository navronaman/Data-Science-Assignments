regday4.pck <-
c("regday4.pck", "regday2.pck", "fixregday1.pck", "regday1.pck", 
"anscombe.fit1", "anscombe.fit2", "gauss.reg1", "NOAAGISSWD", 
"my.hat.w", "bootmatsmooth", "bootmatlin", "gen.inv1", "is.inf", 
"gauss.reg", "gauss.reg1a", "my.hat.w1", "gen.inv.sing", "cv.train.test", 
"cv.train.test0", "kfold.cv", "cv.press", "Fun.in.Tstats", "matrix.2ndorder.make", 
"regday3.pck", "regboot", "generalized.bootstrap", "clt.sim", 
"mrpois", "mrbinom", "sim.coverage", "logitboot", "aov1")
regday2.pck <-
...
function(x,y,nboot=10000,alpha=.05)
{
	...
	for(i in 1:nboot){ #for i in 1 to nboot
		...
		covboot<-(t(x0boot)%*%x0boot) #calculate inverse covariance matrix for bootstrap beta
...
aov1 <-
function(nvec,svec,mvec){
xbb<-sum(nvec*mvec)/sum(nvec)
ssb<-sum(nvec*(mvec-xbb)^2)
sse<-sum((nvec-1)*svec^2)
dfe<-sum(nvec-1)
dfm<-length(mvec)-1
mse<-sse/dfe
msb<-ssb/dfm
fval<-msb/mse
pval<-1-pf(fval,dfm,dfe)
list(ssb=ssb,sse=sse,dfb=dfm,dfe=dfe,mse=mse,msb=msb,F=fval,p=pval)}
